			...
			<table id="register"  class="table table-striped table-bordered mTable" >
				...
				<tbody>
					<c:forEach var="register" items="${requestScope.registers }" varStatus="status">
						<tr><td style="width: 40px;"><input style="margin-top: 0px;margin-left: 10px;" type="checkbox" name="mcheckbox" value="${register.registerId }"></td><td class="id">${register.registerId }</td><td>${register.name }</td><td>${register.phoneNumber }</td><td>${register.email }</td><td><a href="managePersonAction_updateR?registerId=${register.registerId  }">详情</a></td><td><a href="javascript:;" onclick="mdelete(this,2)">删除</a></td></tr>
					</c:forEach>
				</tbody>
			</table>
	</c:otherwise>
