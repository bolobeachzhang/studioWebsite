...
<script type="text/javascript">
	...
	//显示回复框
	function showReplayTable(obj,messager,messageCon,messageDate,messageId){
		...
		$("#pass").click(function(){
			var replayCon = $("#replayCon").val();
			var messageState = $("#messageState option:selected").val();
			$.ajax({
				type: "POST",
				url: "manageMessageAction_replay",
				data: "replayCon="+replayCon+"&messageState="+messageState+"&messageId="+messageId,
				beforeSend: function (){
					$("#loding").show();
				},
				success: function(msg){
...
<body>
	...
	<c:choose>
		...
		<c:otherwise>
				...
				<table class="table table-striped table-bordered mTable">
					<thead><tr>
						<th>选中</th><th>留言者</th><th>留言内容</th><th>留言时间</th><th>回复</th><th>操作</th>
					</tr></thead>
					<tbody>
					<c:forEach var="message" items="${requestScope.messages }" varStatus="status">
						<tr>
							...
							<td><a href="javascript:;" onclick="showReplayTable(this,'${message.messager }','${message.messageCon }','${message.messageDate }',${message.messageId})">回复留言</a></td>
		...
		<c:if test="${requestScope.pageCount != 1}">  
		    ...
		    <c:choose>  
		        ...
		        <c:otherwise>  
		            ...
		            ...&nbsp;  
		            <a href="manageMessageAction_showTUncheck?&pageNow=${requestScope.pageCount}">  
	...
	<table  class="table table-bordered " >
		...
		<tr><td><span style="color: #FFFFFF">回复：</span></td><td><textarea id="replayCon"></textarea></td></tr>
		<tr><td colspan="2">
			<select id="messageState" style="margin-top: 10px;">
				<option value="0">全部人员</option>
				<option value="1">学生、老师和注册人员</option>
				<option value="2">学生和老师</option>
				<option value="4">老师</option>
			</select>
			<button class="btn" id="pass">审核通过</button>
			<button class="btn" id="dele">删除留言</button>
			</td></tr>
	</table>
	</div>
</body>
