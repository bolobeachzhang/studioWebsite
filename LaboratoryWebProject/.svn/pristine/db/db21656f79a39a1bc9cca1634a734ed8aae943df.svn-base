			...
			<table id="manager"  class="table table-striped table-bordered mTable" >
				...
				<tbody>
					<c:forEach var="administrator" items="${requestScope.administrators }" varStatus="status">
						<tr><td style="width: 40px;"><input style="margin-top: 0px;margin-left: 10px;" type="checkbox"  name="mcheckbox" value="${administrator.adminId }"></td><td class="id">${administrator.adminId }</td><td>${administrator.name }</td><td><a href="javascript:;" onclick="mdelete(this,16)">删除</a></td></tr>	
					</c:forEach>
				</tbody>
			</table>
		</c:otherwise>
