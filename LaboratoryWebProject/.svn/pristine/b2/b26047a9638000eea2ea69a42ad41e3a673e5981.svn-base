...
public class ForumServiceImpl extends BasicDaoImpl  implements ForumService {
	...
	public List<Forum> showTopicThemeByPage(int pageNow,int pageSize,int parentId)
	{
		if(parentId == 0)
		{
			String hql = "from Forum where parentId = 0 "+" order by spokerTime desc";
			return this.pageQuery(hql,pageSize*(pageNow - 1), pageSize, null);
		}
		else
		{
			String hql = "from Forum where parentId = "+parentId +" order by spokerTime desc";
			return this.pageQuery(hql,pageSize*(pageNow - 1), pageSize, null);
		}
	...
	{
		String hql = "from Forum where id ="+id +" order by spokerTime desc";
	...
	{
		String hql = "from Forum where parentId ="+id +" order by spokerTime desc";
