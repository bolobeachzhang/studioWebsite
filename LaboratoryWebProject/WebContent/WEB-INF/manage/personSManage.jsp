	...
	<c:choose>
		...
		<c:otherwise>
			<table id="student"  class="table table-striped table-bordered mTable" >
				<thead>
					<tr><th>选择</th><th>学生ID</th><th>学生名字</th><th>班级</th><th>邮箱</th><th>详情</th><th>操作</th></tr>
				</thead>
				<tbody>
					<c:forEach var="student" items="${requestScope.students }" varStatus="status">
						<tr><td style="width: 40px;"><input style="margin-top: 0px;margin-left: 10px;" type="checkbox" name="mcheckbox" value="${student.studentId }"></td><td class="id">${student.studentId }</td><td>${student.name }</td><td>${student.classes }</td><td>${student.email }</td><td><a href="managePersonAction_updateS?studentId=${student.studentId  }">详情</a></td><td><a href="javascript:;" onclick="mdelete(this,4)">删除</a></td></tr>
					</c:forEach>
				</tbody>
			</table>
		</c:otherwise>
		...
		<c:if test="${requestScope.pageCount != 1}">  
		    ...
		    <c:choose>  
		        ...
		        <c:otherwise>  
		            ...
		            ...&nbsp;  
		            <a href="managePersonAction_studentManage?&pageNow=${requestScope.pageCount}">  
		                [${requestScope.pageCount}]</a>&nbsp;  
		        </c:otherwise>  
		    </c:choose>  
		</c:if>  
		<div id="mdelete"><a href="javascript:;" onclick="pickadd(this)">全选</a> <a href="javascript:;" onclick="deleteChecked(2)">删除选中</a></div>
	</div>
</body>
</html>