	...
	<c:choose>
		...
		<c:otherwise>
			<table class="table table-striped table-bordered mTable">
				<thead>
					<tr><th>选中</th><th>标题</th><th>类型</th><th>期刊名</th><th>作者</th><th>审核状态</th><th>详情</th><th>操作</th></tr>
				</thead>
				<tbody>
				<c:forEach var="thesis" items="${requestScope.thesiss }" varStatus="status">
					<tr><td style="width: 40px;"><input style="margin-top: 0px;margin-left: 10px;" type="checkbox" name="mcheckbox" value="${thesis.id }" url="${thesis.url }"></td> 
						...
						<td><a href="">查看</a></td>
						<td><a href="manageThesisAction_updateT?id=${thesis.id }">修改</a>&nbsp;&nbsp;<a href="javascript:;" onclick="mdelete(${thesis.id },'${thesis.url}',this)">删除</a></td></tr>
				</c:forEach>
				</tbody>
			</table>
		</c:otherwise>
		...
		<c:if test="${requestScope.pageCount != 1}">  
		    ...
		    <c:choose>  
		        ...
		        <c:otherwise>  
		            ...
		            ...&nbsp;  
		            <a href="manageThesisAction_getT?&pageNow=${requestScope.pageCount}">  
