...
<!-- ==================================分页开始======================================== -->
	<div id="paging">
		...
		<c:if test="${requestScope.pageNow >1}">
			<a fff="frontDynamicAction_part?section=4&pageNow=${requestScope.pageNow-1}" onclick="pageQuery(this)" style="cursor:pointer;">上一页</a>&nbsp;  
		</c:if>
		<a fff="frontDynamicAction_part?section=4&pageNow=1" onclick="pageQuery(this)" style="cursor:pointer;">[1]</a>&nbsp;  
		<c:if test="${requestScope.pageCount != 1}">  
		    <c:choose>  
		        <c:when test="${requestScope.pageNow <= 5}">  
		            <c:forEach var="i" begin="2" end="${requestScope.pageNow}">  
		                <a fff="frontDynamicAction_part?section=4&pageNow=${i}" onclick="pageQuery(this)" style="cursor:pointer;">[${i }]</a>&nbsp;  
		            </c:forEach>  
		        </c:when>  
		        <c:otherwise>  
		            ...&nbsp;  
		    ...
		    <c:choose>  
		        ...
		        <c:otherwise>  
		            ...
		            ...&nbsp;  
		            <a fff="frontDynamicAction_part?section=4&pageNow=${requestScope.pageCount}" onclick="pageQuery(this)" style="cursor:pointer;">  
		                [${requestScope.pageCount}]</a>&nbsp;  
		        </c:otherwise>  
		    </c:choose>  
		</c:if>  
		<c:if test="${requestScope.pageNow < requestScope.pageCount}">
			<a fff="frontDynamicAction_part?section=4&pageNow=${requestScope.pageNow+1}" onclick="pageQuery(this)" style="cursor:pointer;">下一页</a>&nbsp;  
		</c:if>
	</div>
	<!-- ==================================分页结束======================================== -->
	</div>
